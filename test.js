...
try {
   ...
   *    1.1. if $TESTCASES_AC value is empty: build execute command that execute all the tests
   *    1.2. if $TESTCASES_AC value is NOT empty: obtains test names in $TESTCASES_AC then build execute command that execute those tests only
   */
  ...
  if ($TESTRUNS_LIST != undefined && $TESTRUNS_LIST.trim() != null) {
    ...
    testCommand = `./node_modules/.bin/cypress run --env tests='testruns_list.json' --browser chrome --reporter junit --reporter-options='mochaFile=reports/junit-report-[hash].xml,toConsole=true'`;
  }
